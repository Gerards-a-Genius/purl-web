...
// Pattern cache
let patternCache: Map<string, any> = new Map();
 ...
 */
function loadPattern(patternId: string): any {
...
function searchPatterns(
  ...
  let results = catalog.patterns.filter((p: any) => {
    // Text search
    const matchesQuery =
  ...
  results = results.slice(0, limit).map((p: any) => {
 ...
 * Get pattern by ID
 */
function getPattern(patternId: string): any {
    ...
    tools: [
      {
        ...
        description:
          "Search the pattern repository by keyword, technique, or attributes. Returns matching patterns with metadata.",
        inputSchema: {
          type: "object",
          properties: {
            query: {
      ...
      },
      {
        name: "get_pattern",
        description:
          "Get full details for a specific pattern by ID, including materials, instructions, and metadata.",
        inputSchema: {
          type: "object",
          properties: {
            pattern_id: {
      ...
      },
      {
        ...
        description:
          "List all knitting/crochet techniques found across patterns in the repository, with counts.",
        inputSchema: {
          type: "object",
          properties: {},
        },
      },
      {
        ...
        description:
          "Get statistics about the pattern repository including total counts and breakdowns.",
        inputSchema: {
          type: "object",
          properties: {},
        },
      },
      {
        ...
        description:
          "Find patterns similar to a given pattern based on techniques and attributes.",
        inputSchema: {
          type: "object",
          properties: {
            pattern_id: {
  ...
  try {
    switch (name) {
              ...
              text: JSON.stringify(
                {
                  ...
                  patterns: results.map((p) => ({
                    ...
                    techniques: p.techniques?.map((t: any) => t.name),
                  })),
                },
                null,
                2
      ...
      case "get_similar_patterns": {
        const sourcePattern = getPattern(args?.pattern_id as string);
        ...
        const sourceTechniques = new Set(
          sourcePattern.techniques?.map((t: any) => t.name) || []
        );

        const catalog = loadCatalog();
        const similar = catalog.patterns
          .filter((p: any) => p.id !== args?.pattern_id)
          .map((p: any) => {
            const pattern = loadPattern(p.id);
            if (!pattern) return null;

            const patternTechniques = new Set(
              pattern.techniques?.map((t: any) => t.name) || []
            );

            // Calculate similarity score
            let overlap = 0;
            for (const tech of sourceTechniques) {
              if (patternTechniques.has(tech)) overlap++;
            }
          ...
          .map((r: any) => ({
            ...
            techniques: r.pattern.techniques?.map((t: any) => t.name),
          }));
